...
async def upload_file_endpoint(
    ...
    return await create_file(session=session, file_data=file_data, file=file, course_id=course_id, lesson_id=lesson_id)
...
async def get_files_endpoint(course_id: int, lesson_id: int, session: AsyncSession = Depends(get_session)):
    ...
    if result:
        return result
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Файлов нет')
