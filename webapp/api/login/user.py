...
from webapp.schema.login.user import User as PydanticUser, UserCreate
...
async def create_user_endpoint(user_data: UserCreate, current_user: PydanticUser = Depends(get_current_user)):
    # Предполагается, что операцию может выполнить только аутентифицированный пользователь
...
async def update_user_endpoint(
    ...
    # Проверка прав пользователя, если необходимо
